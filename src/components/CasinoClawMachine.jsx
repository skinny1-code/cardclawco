...
// Phase: idle → inserting → descending → grabbing → rising → delivering → done
const TIER_MACHINE = {
  CoreClaw: {
    cabinetColor:  '#0a1628',
    accentColor:   '#3B82F6',
...
// SVG Claw — number of fingers scales with tier
function ClawSVG({ tier, phase, color }) {
  ...
  const spreadAngle = grabbing ? 8 : 30
  ...
  return (
    <svg width="60" height="50" viewBox="0 0 60 50" style={{ overflow:'visible', filter:`drop-shadow(0 0 8px ${color})` }}>
      ...
      {Array.from({length: fingers}).map((_, i) => {
        const total = fingers
        const step = (spreadAngle * 2) / (total - 1 || 1)
        ...
        return (
          <path key={i}
            d={`M30,12 Q${cx},${cy} ${x2},${y2}`}
            stroke={color} strokeWidth={tier === 'QuantumClaw' ? 3.5 : 3}
...
export default function CasinoClawMachine({ tier, onPullComplete, disabled, pulls }) {
  ...
  const [clawY, setClawY] = useState(0)       // 0 = top, 1 = bottom
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setLightsOn(v => !v || Math.random() > 0.3)
    }, 600)
  ...
  const runPull = async () => {
    ...
    const rise = () => {
      const t = Math.max(1 - (Date.now() - start2) / 1500, 0)
      setClawY(t)
  ...
  return (
    <div style={{ width:'100%', maxWidth:320, margin:'0 auto', userSelect:'none' }}>
      <style>{`
        ...
        @keyframes insertCoin { 0%{transform:translateY(-20px);opacity:0} 100%{transform:translateY(0);opacity:1} }
      `}</style>
          ...
          {[0,1].map(k => (
            <div key={k} style={{ flex:'0 0 50%', display:'flex', gap:40, paddingRight:40 }}>
              {['🎰 PULL TO WIN','✨ GRADED CARDS','💎 NFT OWNERSHIP','🏆 TOP COLLECTORS','🎰 PULL TO WIN','✨ GRADED CARDS'].map((t,i) => (
                <span key={i} style={{ fontFamily:"'Oswald',sans-serif", fontSize:10, fontWeight:700, letterSpacing:2, color:cfg.neonColor }}>
                  {t}
                </span>
              ))}
            </div>
      ...
      {/* Main cabinet */}
      <div style={{
        background: cfg.cabinetGrad,
        border: `2px solid ${cfg.accentColor}60`,
        borderRadius: '0 0 16px 16px',
        padding: 16,
        position: 'relative',
        boxShadow: `0 0 40px ${cfg.glowColor}, inset 0 0 60px rgba(0,0,0,0.5)`,
        animation: phase === 'grabbing' ? 'machineShake 0.3s ease' : 'none',
      }}>
        ...
        <div style={{ textAlign:'center', marginBottom:10, position:'relative', zIndex:1 }}>
          <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:11, fontWeight:700, letterSpacing:4, color:cfg.neonColor, textShadow:`0 0 10px ${cfg.neonColor}` }}>
            {cfg.label}
          </div>
        ...
        <div style={{
          height: glassH,
          background: 'linear-gradient(180deg,rgba(0,0,0,0.8) 0%,rgba(0,20,40,0.9) 100%)',
          border: `2px solid ${cfg.accentColor}50`,
          borderRadius: 10,
          position: 'relative',
          overflow: 'hidden',
          marginBottom: 14,
          boxShadow: `inset 0 0 30px rgba(0,0,0,0.6), inset 0 0 10px ${cfg.accentColor}20`,
        }}>
          {/* Glass reflection */}
          ...
          {/* Cards preview in cabinet */}
          <div style={{ position:'absolute', bottom:12, left:0, right:0, display:'flex', justifyContent:'space-around', alignItems:'flex-end', padding:'0 10px' }}>
            {[...Array(3)].map((_, i) => (
          ...
          <div style={{
            ...
            transition: phase === 'idle' ? 'top 0.5s ease' : 'none',
            zIndex:10,
          }}>
            {/* Arm down to claw */}
          ...
          {/* Phase overlays */}
          {phase === 'inserting' && (
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', animation:'insertCoin 0.3s ease' }}>
              <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:13, color:cfg.neonColor, letterSpacing:3, textShadow:`0 0 20px ${cfg.neonColor}` }}>INSERT COIN…</div>
            </div>
          )}
          {phase === 'delivering' && (
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', background:`radial-gradient(circle,${cfg.accentColor}30 0%,transparent 70%)` }}>
              <div style={{ fontFamily:"'Oswald',sans-serif", fontSize:13, color:cfg.neonColor, letterSpacing:3, animation:'coinGlow 0.5s ease infinite' }}>GOT ONE! 🎉</div>
            </div>
          )}
        </div>

        {/* Control panel */}
        <div style={{ background:'rgba(0,0,0,0.4)', border:`1px solid ${cfg.accentColor}30`, borderRadius:10, padding:'12px 16px', position:'relative' }}>
          {/* Pulls available display */}
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10 }}>
            ...
            <div style={{ display:'flex', gap:6 }}>
              {[...Array(Math.min(pulls || 0, 5))].map((_,i) => (
                <div key={i} style={{ width:10, height:10, borderRadius:'50%', background:cfg.neonColor, boxShadow:`0 0 8px ${cfg.neonColor}` }}/>
              ))}
              {pulls === 0 && <div style={{ fontFamily:"'DM Mono',monospace", fontSize:9, color:'rgba(240,237,230,0.2)' }}>NO PULLS</div>}
            </div>
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: isActive && !isRunning
                ? `linear-gradient(135deg,${cfg.accentColor},${cfg.accentColor}80)`
                : 'rgba(255,255,255,0.05)',
              border: `2px solid ${isActive ? cfg.accentColor : 'rgba(255,255,255,0.08)'}`,
              borderRadius: 10,
              ...
              fontWeight: 700,
              letterSpacing: 3,
              color: isActive && !isRunning ? '#0a0a0a' : 'rgba(240,237,230,0.2)',
              textShadow: 'none',
              boxShadow: isActive && !isRunning ? `0 0 20px ${cfg.accentColor}, 0 4px 15px rgba(0,0,0,0.5)` : 'none',
              transition: 'all 0.2s',
              animation: isActive && !isRunning ? `coinGlow 2s ease-in-out infinite` : 'none',
            }}
          >
            {isRunning ? (
              phase === 'inserting' ? 'INSERTING…' :
              phase === 'descending' ? 'DESCENDING…' :
              phase === 'grabbing' ? '⊂(◉‿◉)つ GRABBING…' :
              phase === 'rising' ? 'RISING…' : 'DELIVERING…'
            ) : isActive ? `🎰 PULL NOW` : `BUY A PULL · $${
              tier==='CoreClaw'?25:tier==='PremierClaw'?50:tier==='UltraClaw'?100:500
            }`}
          </button>
        </div>
      </div>
